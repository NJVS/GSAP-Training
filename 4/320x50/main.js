document.addEventListener('DOMContentLoaded', () => {
  ...
  mainTimeline
    ...
    .set('.text image', { autoAlpha: 0, y: 15 })
    .set('.rect-top rect', { x: '100%' })
    .set('.specialoffer rect', { autoAlpha: 0 })
    .set('.specialoffer .border-left', { x: 66, scaleY: 0 })
    .set('.specialoffer .border-right', { x: -66, scaleY: 0 })
...
const rectYPositions = ['-60%', '-65%', '-70%']
const rectInOutProps = {
  x: index => rectYPositions[index],
  stagger: rectStagger,
  ease: 'power4.inOut'
}
const rectTopProps = {
...
const f4Timeline = () => {
  return gsap.timeline()
    ...
    // top rect
    .to('.frame4 .rect-top rect', rectSpeed, rectTopProps, `<+0.2`)
    // heading
    ...
    .to('.frame4 .specialoffer rect', 0.2, { y: 0, scaleY: 1 })
    // specialoffer border back to original position
    .to('.frame4 .specialoffer rect', 0.3, { x: 0 })
